...
confirmed_cases<-data.table(read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"))
...
confirmed_cases_long <- melt(confirmed_cases,
                             ...
                             value.name = "Confirmed Fatalities"
                             )

confirmed_cases_long[,SelectedRegion:=`Country/Region`]
#confirmed_cases_long[`Country/Region`=="China",SelectedRegion:=paste0(`Province/State`," (USA)")]
#confirmed_cases_long[`Country/Region`=="China",SelectedRegion:=paste0(`Province/State`," (China)")]
#confirmed_cases_long[`Country/Region`=="Italy",SelectedRegion:=paste0(`Province/State`," (Italy)")]

confirmed_cases_long_bycountry <- confirmed_cases_long[,.(`Confirmed Fatalities`=sum(`Confirmed Fatalities`)),by=.(SelectedRegion,case_date)]
...
ggplot(confirmed_cases_long_bycountry[SelectedRegion %in% c("New Zealand","Australia","Canada")],
       aes(x=case_date,y=`Confirmed Fatalities`,group=SelectedRegion,color=SelectedRegion))+
  geom_line() + 
...
confirmed_cases_long_bycountry[,IsAtOrPastThreshold:=(`Confirmed Fatalities`>=threshold)]
...
```{r}
    ...
    threshold<-10
     ...
     if(threshold<min(confirmed_graph[SelectedRegion=="China",`Confirmed Fatalities`])){
       confirmed_graph <- 
     ...
     country_labels<-confirmed_graph[,
       ...
       merge(confirmed_graph[,.(SelectedRegion,DaysPastThreshold,`Confirmed Fatalities`,DatePastThreshold)],
     ...
     myplot<-ggplot(confirmed_graph
            ...
            #                                    "Hong Kong","US")]
            ,
            aes(x=DaysPastThreshold,y=`Confirmed Fatalities`,
       ...
       geom_text(data=country_labels,
                 aes(x=intercept_day,y=`Confirmed Fatalities`,label=SelectedRegion),
...
Can we plot the number of New Fatalities by day?


```{r}
confirmed_cases_long_bycountry[order(case_date),
                               `New Fatalities`:=(`Confirmed Fatalities`-shift(`Confirmed Fatalities`)),
                               SelectedRegion]
  
#moving average over 3 days; figure is dated on the last day of the average window.
confirmed_cases_long_bycountry[order(case_date),
                               NewCasesMA:=c(rep(NA,2),(zoo::rollapply(`New Fatalities`,3,mean))),
                               SelectedRegion]

new_cases<-confirmed_cases_long_bycountry[!is.na(NewCasesMA)]

if(threshold<min(new_cases[SelectedRegion=="Mainland China",`Confirmed Fatalities`])){
       new_cases <- 
...
myplot<-ggplot(new_cases
 ...
 labs(title=paste0("Growth in new cases by day from the ",threshold," case mark, by country"),
      x=paste0("Days past first day with ",threshold," cases"),
      y="New Fatalities (3 day rolling average)",
      color="Date Country Reached Threshold")+
 theme(legend.position = "none")+
 geom_text(data=country_labels,
...
confirmed_cases_long_bycountry[order(case_date),
                               #NewCasesMA:=shift(zoo::rollapply(`New Fatalities`,7,mean),0),
                               #c(rep(NA,2),(zoo::rollapply(`New Fatalities`,3,mean)))
                               NewCasesMA_c:=zoo::rollapply(`New Fatalities`,7,mean,fill=c(NA,NA,NA)),
                               SelectedRegion]

confirmed_cases_long_bycountry[order(case_date),
                               #NewCasesMA:=shift(zoo::rollapply(`New Fatalities`,7,mean),0),
                               NewCasesMA:=NewCasesMA_c,#shift(NewCasesMA_c,2),
                               SelectedRegion]
...
myplot<-ggplot(new_cases
 ...
 coord_cartesian(xlim=c(as.Date("2020-02-20"),max(new_cases$case_date)+7),
                 ylim=c(5,max(new_cases$NewCasesMA)))+
 scale_y_continuous(trans = "log10",
 ...
 #geom_vline(xintercept = 0,linetype="dashed")+
 #geom_hline(yintercept = threshold,,linetype="dashed")+
 labs(title=paste0("New fatalities by day and country"),
      x=paste0("Date"),
      y="New Fatalities (7 day rolling average)",
      color="Date Country Reached Threshold",
...
new_cases<-confirmed_cases_long_bycountry[!is.na(NewCasesMA) & 
                                            SelectedRegion %in% c("United Kingdom","United States","Russia", "Sweden","Denmark","Belgium",
                                                                  "France","Spain","Italy","Finland","Germany")]
...
country_labels_grouped<- country_labels[,
                                        ...
                                        ,by=.(NewCasesGroup,intercept_date)] %>% .[GroupedNewCases>1]
x_breaks  <- seq(min(new_cases$case_date),max(new_cases$case_date),7)
...
myplot<-ggplot(new_cases
 ...
 scale_x_date(#breaks=x_breaks,
   date_breaks = "7 days",
                    minor_breaks=NULL,date_labels="%b %d")+
 coord_cartesian(xlim=c(max(new_cases$case_date)-days_limit,max(new_cases$case_date)+5),
                 ylim=c(5,max(new_cases$NewCasesMA)*1.5))+
 scale_y_continuous(trans = "log10",
                    breaks = c(2,5,10,20,50,100,200,500,1000,2000,5000,10000,20000,50000,100000),
                    minor_breaks=NULL,
                    labels = scales::comma,
 ...
 #geom_vline(xintercept = 0,linetype="dashed")+
 #geom_hline(yintercept = threshold,,linetype="dashed")+
 labs(title=paste0("New fatalities by day and country or Chinese region"),
      x=paste0("Date"),
      y="New Fatalities (7 day rolling average)",
      color="Date Country Reached Threshold",
      caption="Source: John Hopkins Dataset")+
